...
public class Sail : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        {
            // Normalna żagla (zielona) - w płaszczyźnie XZ
            Gizmos.color = Color.green;
            Vector3 worldPos = transform.position;
            Vector3 sailNormal = transform.forward * sailLength * 0.5f;
            sailNormal.y = 0;
            Gizmos.DrawLine(worldPos, worldPos + sailNormal);
            
            // Oś obrotu (żółta) - pionowa
    ...
    {
        // Wizualizacja zakresu obrotu w płaszczyźnie XZ
        Gizmos.color = Color.cyan;
        Vector3 pos = Application.isPlaying ? transform.position : transform.position;
        
        // Pobierz rotację rodzica (jachtu) jeśli istnieje
        ...
        // Linia min kąta
        Quaternion minRot = parentRotation * Quaternion.Euler(0, minAngle, 0);
        Vector3 minDir = minRot * Vector3.forward * sailLength * 0.7f;
        Gizmos.DrawLine(pos, pos + minDir);
        
        // Linia max kąta
        Quaternion maxRot = parentRotation * Quaternion.Euler(0, maxAngle, 0);
        Vector3 maxDir = maxRot * Vector3.forward * sailLength * 0.7f;
        Gizmos.DrawLine(pos, pos + maxDir);
        
        // Łuk pokazujący zakres
        DrawArc(pos, minAngle, maxAngle, sailLength * 0.7f, parentTransform);
        
        // Pokaż aktualny kąt jeśli w play mode
        if (Application.isPlaying)
        {
            Gizmos.color = Color.yellow;
            Quaternion currentRot = parentRotation * Quaternion.Euler(0, currentAngle, 0);
            Vector3 currentDir = currentRot * Vector3.forward * sailLength * 0.9f;
            Gizmos.DrawLine(pos, pos + currentDir);
        }
    }
    ...
    void DrawArc(Vector3 center, float startAngle, float endAngle, float radius, Transform referenceTransform)
    {
        ...
        // Oblicz różnicę kątów (zawsze w zakresie -180..180)
        float angleDiff = endAngle - startAngle;
        
        // Jeśli różnica > 180, to łuk przechodzi "przez tył"
        if (angleDiff > 180f)
        {
            angleDiff -= 360f;
        }
        ...
        // Bazowa rotacja z rodzica (jachtu)
